...
L["This mode will directly append the macro text onto an automatically generated command generated by other bindings using the specified keybind. Generally, this means that it will be the last section of an '/use' command."] = "此模式将直接把宏文本附加到自动生成的命令后, 这意味着你填写的内容是 '/use' 命令的后面部分."
L["With this mode you're not writing a macro command. You're adding parts to an already existing command, so writing '/use Holy Light' will not work, in order to cast Holy Light simply type 'Holy Light'."] = "所以此模式下你不用写入宏命令. 你是在为已存在的命令增加组件,  '/use 圣光术'将不能运行, 要施放圣光术只需要简单的输入'圣光术'."
...
L["No Instance"] = "不在副本"
L["Zone name(s)"] = "区域名称"
L["Comma separated, use an exclamation mark (%s) to inverse a zone condition, for example:"] = "用逗号分隔, 使用感叹号 (%s) 反转区域条件, 例如:"
